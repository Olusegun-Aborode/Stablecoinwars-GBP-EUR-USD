...
def parse_spl_transfer_instruction(instruction_data: bytes) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if instruction_type in [3, 12]:
            ...
            return {'amount': amount, 'type': instruction_type}
    ...
    try:
        ...
        try:
            ...
            if not signatures_response.value:
                ...
                # Count signatures as proxy for transfers
                transfer_count = len(signatures_response.value)

                # Compute actual volume by decoding TransferChecked instructions for this mint
                sigs: List[Signature] = signatures_response.value
                transfer_volume = get_detailed_transfer_volume(client, sigs, decimals, address)
                logger.debug(f"Found {transfer_count} recent transactions for {symbol}; computed volume: {transfer_volume}")
...
def get_detailed_transfer_volume(client: Client, signatures: List[Signature], decimals: int, mint: str) -> float:
    """
    ...
    # Limit to first 150 signatures to balance accuracy and rate limits
    for sig in signatures[:150]:
        try:
            ...
            # accountKeys can be a list of strings or objects with 'pubkey'
            raw_account_keys = message.get('accountKeys', [])
            account_keys: List[str] = []
            for k in raw_account_keys:
                if isinstance(k, dict):
                    # Newer encoding includes pubkey field
                    pk = k.get('pubkey')
            ...
            for ix in instructions:
                try:
                    ...
                    parsed = parse_spl_transfer_instruction(instruction_data)
                    if not parsed:
                        continue

                    # For TransferChecked (12), accounts layout: [source, mint, destination, owner, ...]
                    if parsed.get('type') == 12:
                        ...
                        if isinstance(mint_idx, int) and mint_idx < len(account_keys):
                            ...
                            if mint_addr == mint:
                                total_volume += parsed['amount']
                    # For Transfer (3), mint is not directly referenced; skip to avoid miscounting
                except Exception as inner_e:
                    logger.debug(f"Instruction parse failure: {str(inner_e)}")
                    continue
