...
def extract_all_chains() -> List[Dict[str, Any]]:
    ...
    for chain in ['ethereum']:
        ...
        try:
            ...
            send_alert(f"Extraction failed for {chain}: {str(e)}")
    ...
    if 'solana' in STABLECOINS:
        ...
        try:
            ...
            send_alert(f"Extraction failed for Solana: {str(e)}")
    ...
    try:
        ...
        for metric in metrics:
            if validate_metrics(metric):
                ...
                send_alert(f"Data validation issue: {metric['coin']} on {metric['chain']}")
        ...
        send_alert(f"CRITICAL: Extraction job failed - {str(e)}")
        sys.exit(1)
